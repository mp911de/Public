...
public class HttpControlInterface {
    ...
    public String controlPowerflare(@PathParam("port")
                                    byte port, @PathParam("state")
                                    String state) {
     ...
     * @return OK/FAIL
     */
    @Path("powerflareUsbPower/{port:[1-4]}/{state:(ON|OFF)}")
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public String controlPowerflareUsbPower(@PathParam("port")
                                            byte port, @PathParam("state")
                                            String state) {

        System.out.println("controlPowerflareUsbPower port|" + port + "|state|" + state);
        ...
        AbstractCommunicationCommand command = null;
        if (state.equalsIgnoreCase("ON")) {
            command = new PowerflareUsbPowerSwitchOnCommand(port);
        }

        if (state.equalsIgnoreCase("OFF")) {
            command = new PowerflareUsbPowerSwitchOffCommand(port);
        }
    ...
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public String controlSwitch(@PathParam("port")
                                byte port) {
    ...
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public String controlPort(@PathParam("port")
                              byte port, @PathParam("state")
                              String state) {
